      ...
      <div className="progress-bar">
        ...
        <div className="status-items">
          {statuses.map((status, index) => (
            <div
              key={index}
              className={`status-item ${index <= currentIndex ? 'active' : ''}`}
              style={{
                left: `${(index / (statuses.length)) * 112}%`, // Adjusting the position of status items
              }}
            ...
            >
              {currentStatus === status.value && (
                <div
                  ...
                  style={{
                    left: `${(index / (statuses.length - 1)) * 100}%`, // Move the truck icon to the current status
                  }}
                >
